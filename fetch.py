...
class Fetch:
    ...
    def fetch_video(self, video_id, index):
        if index == 1:
            cmd = [
                ...
                QUALITIES[index],
                "-o",
                OUTDIRS[index] + "%(title)s.%(ext)s",
                "--",
                video_id,
            ]
        else:
            cmd = [BINARY, "--restrict-filenames", "-f", QUALITIES[index], "-o", OUTDIRS[index] + "%(title)s.%(ext)s", "--", video_id]
    ...
    def main(self):
        for i in range(len(PLAYLIST_IDS)):
            ...
            request = youtube.playlistItems().list(part="snippet", playlistId=PLAYLIST_IDS[i], maxResults=50)
